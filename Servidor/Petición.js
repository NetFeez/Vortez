...
export default class Petición {
	...
	Cookies = null;
	/**@type {Promise<import('../Tipo').NetFeez-Labs.Servidor.Petición.GET>} Contiene los datos POST que se enviaron. */
	GET = null;
	/**@type {string} Contiene la dirección IP de quien realizo la petición. */
	IP = null;
	/**@type {import('../Tipo').NetFeez-Labs.Servidor.Petición.Método} Contiene el método de la petición. */
	Método = null;
	/**@type {Promise<import('../Tipo').NetFeez-Labs.Servidor.Petición.POST>} Contiene los datos POST que se enviaron. */
	POST = null;
	/**@type {import('../Tipo').NetFeez-Labs.Servidor.Sesión} */
	Sesión = null;
	...
	Url = null;
	/**
	 * Crea la forma de petición de `NetFeez-Labs/Servidor`.
	 ...
	 * @param {import('http').IncomingMessage} SrvPetición La petición que recibió el servidor.
	 * @returns {Promise<import('../Tipo').NetFeez-Labs.Servidor.Petición.POST>}
	 */
	Datos_Post(SrvPetición) {
		return new Promise((PrRespuesta, PrError) => {
			let Datos = Buffer.from([]);
			let Partes = [];
			/**@type {import('../Tipo').NetFeez-Labs.Servidor.Petición.POST} */
			let POST = {
				Archivos: new Map,
				Formato: 'Desconocido',
				Variables: new Map
	 ...
	 * @returns {import('../Tipo').NetFeez-Labs.Servidor.Petición.Método}
	 */
